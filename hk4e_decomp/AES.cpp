...
void AES::SubBytes(unsigned char state[4][Nb]) {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < Nb; j++) {
      t = state[i][j];
      state[i][j] = state[i][j] ^ sbox[t / 16][t % 16];
    }
  }
}
...
void AES::KeyExpansion(const unsigned char key[], unsigned char w[]) {
  ...
  static const uint8_t power_schedule[10] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
  ...
  auto out_ptr = w + 0x1f;
  uint8_t a, b, c, d, ttemp = 0;
  for (int i = 0; i < 10; i++) {
      a = sbox[out_ptr[-0x14] / 16][out_ptr[-0x14] % 16];
      b = sbox[out_ptr[-0x10] / 16][out_ptr[-0x10] % 16];
      c = sbox[out_ptr[-0x18] / 16][out_ptr[-0x18] % 16] ^ out_ptr[-0x18] ^ power_schedule[i] ^ out_ptr[-0x1f];
      d = sbox[out_ptr[-0x1c] / 16][out_ptr[-0x1c] % 16];

      out_ptr[-0xf] = c;
      ttemp = a ^ out_ptr[-0x14] ^ out_ptr[-0x1b];
      out_ptr[-0xb] = ttemp;
      a = b ^ out_ptr[-0x10] ^ out_ptr[-0x17];
      out_ptr[-7] = a;
      b = d ^ out_ptr[-0x1c] ^ out_ptr[-0x13];
      out_ptr[-3] = b;
      c = c ^ out_ptr[-0x1e];
      out_ptr[-0xe] = c;
      ttemp = ttemp ^ out_ptr[-0x1a];
      out_ptr[-10] = ttemp;
      a = a ^ out_ptr[-0x16];
      out_ptr[-6] = a;
      b = b ^ out_ptr[-0x12];
      ...
      c = c ^ out_ptr[-0x1d];
      out_ptr[-0xd] = c;
      ttemp = ttemp ^ out_ptr[-0x19];
      out_ptr[-9] = ttemp;
      a = a ^ out_ptr[-0x15];
      out_ptr[-5] = a;
      b = b ^ out_ptr[-0x11];
      out_ptr[-1] = b;
      out_ptr[-0xc] = c ^ out_ptr[-0x1c];
      out_ptr[-8] = ttemp ^ out_ptr[-0x18];
      out_ptr[-4] = a ^ out_ptr[-0x14];
      out_ptr[0] = b ^ out_ptr[-0x10];
